/**
 * ETL — Fato Empenho carga incremental (SQL Server APC -> PostgreSQL local)
 *
 * Estratégia:
 *   1. Busca a última (ano_remessa, numero_remessa) carregada em public.fato_empenho
 *   2. Seleciona as últimas LOOKBACK_REMESSAS remessas a partir dela (janela deslizante)
 *   3. Se o destino estiver vazio, carrega todas as remessas disponíveis (carga inicial)
 *   4. UPSERT por id_despesa: ON CONFLICT (id_despesa) DO UPDATE SET
 *   5. Grava audit.etl_carga (iniciarCargaEtl / finalizarCargaEtl) e audit.etl_log
 *
 ...
 *   FATO_EMPENHO_LOOKBACK_REMESSAS   — nº de remessas a reprocessar a partir da última (default: 3)
 *   FATO_EMPENHO_SUPABASE_BATCH      — tamanho do lote de upsert (default: 500)
 *
 * Uso:
...
const LOOKBACK_REMESSAS = toPositiveInt(
  Number(process.env.FATO_EMPENHO_LOOKBACK_REMESSAS || "3"),
  3,
);
const PG_UPSERT_BATCH = toPositiveInt(
  Number(process.env.FATO_EMPENHO_SUPABASE_BATCH || "500"),
  500,
);
...
// ---------------------------------------------------------------------------
// Seleção de remessas (watermark + lookback)
// ---------------------------------------------------------------------------
...
async function escolherRemessasCarga(): Promise<{ remessas: Remessa[]; modoCarga: "inicial" | "incremental" }> {
  const remessasFonte = await buscarRemessasFonte();
  if (remessasFonte.length === 0) return { remessas: [], modoCarga: "incremental" };

  const ultimaDestino = await buscarUltimaRemessaDestino();

  // Destino vazio: carga inicial completa
  if (!ultimaDestino) {
    return { remessas: remessasFonte, modoCarga: "inicial" };
  }

  // Incremental: seleciona remessas >= última carregada
  const chaveUltima = chaveRemessa(ultimaDestino);
  const remessasElegiveis = remessasFonte.filter((r) => chaveRemessa(r) >= chaveUltima);

  // Aplica lookback: retém as últimas LOOKBACK_REMESSAS contando do final
  if (remessasElegiveis.length > LOOKBACK_REMESSAS) {
    return {
      remessas: remessasElegiveis.slice(remessasElegiveis.length - LOOKBACK_REMESSAS),
      modoCarga: "incremental",
    };
  }
  return { remessas: remessasElegiveis, modoCarga: "incremental" };
}

// ---------------------------------------------------------------------------
// Leitura da remessa na fonte
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Escrita: upsert em lotes
// ---------------------------------------------------------------------------

async function upsertEmLotes(rows: EmpenhoRow[]): Promise<void> {
  for (let i = 0; i < rows.length; i += PG_UPSERT_BATCH) {
    const batch = rows.slice(i, i + PG_UPSERT_BATCH);
    for (const r of batch) {
      await pgQuery(
        `INSERT INTO public.fato_empenho (
           id_despesa, id_remessa, ano_remessa, numero_remessa, id_entidade, id_acao, id_programa,
           id_unidade_orcamentaria, id_fonte_destinacao_recurso, id_aplicacao, numero_funcao,
           numero_subfuncao, numero_categoria_economica, numero_grupo_natureza_despesa,
           numero_modalidade_aplicacao, numero_elemento_despesa, cpf_cnpj_credor, tipo_credor,
           numero_empenho, ano_empenho, data_empenho, tipo_empenho, numero_empenho_ref,
           tipo_lancamento, historico_empenho, valor_empenho, valor_anulado, valor_liquidado,
           valor_pago, valor_retido, valor_empenhado_liquido, valor_a_liquidar, valor_a_pagar,
           etl_atualizado_em
         ) VALUES (
         ...
         ON CONFLICT (id_despesa) DO UPDATE SET
           id_remessa=$2, ano_remessa=$3, numero_remessa=$4, id_entidade=$5, id_acao=$6,
           id_programa=$7, id_unidade_orcamentaria=$8, id_fonte_destinacao_recurso=$9,
           id_aplicacao=$10, numero_funcao=$11, numero_subfuncao=$12,
           numero_categoria_economica=$13, numero_grupo_natureza_despesa=$14,
           numero_modalidade_aplicacao=$15, numero_elemento_despesa=$16, cpf_cnpj_credor=$17,
           tipo_credor=$18, numero_empenho=$19, ano_empenho=$20, data_empenho=$21,
           tipo_empenho=$22, numero_empenho_ref=$23, tipo_lancamento=$24, historico_empenho=$25,
           valor_empenho=$26, valor_anulado=$27, valor_liquidado=$28, valor_pago=$29,
           valor_retido=$30, valor_empenhado_liquido=$31, valor_a_liquidar=$32, valor_a_pagar=$33,
           etl_atualizado_em=$34`,
        [
          r.id_despesa, r.id_remessa, r.ano_remessa, r.numero_remessa, r.id_entidade,
          r.id_acao, r.id_programa, r.id_unidade_orcamentaria, r.id_fonte_destinacao_recurso,
          r.id_aplicacao, r.numero_funcao, r.numero_subfuncao, r.numero_categoria_economica,
          r.numero_grupo_natureza_despesa, r.numero_modalidade_aplicacao, r.numero_elemento_despesa,
          r.cpf_cnpj_credor, r.tipo_credor, r.numero_empenho, r.ano_empenho, r.data_empenho,
          r.tipo_empenho, r.numero_empenho_ref, r.tipo_lancamento, r.historico_empenho,
          r.valor_empenho, r.valor_anulado, r.valor_liquidado, r.valor_pago, r.valor_retido,
          r.valor_empenhado_liquido, r.valor_a_liquidar, r.valor_a_pagar, r.etl_atualizado_em,
        ],
      );
    }
  }
...
export async function executarETLFatoEmpenho(): Promise<void> {
  ...
  console.log(`  -> Destino PostgreSQL: public.fato_empenho`);
  console.log(`  -> Lookback: ${LOOKBACK_REMESSAS} remessa(s)`);
  console.log(`  -> Batch size upsert: ${PG_UPSERT_BATCH}`);
  ...
  try {
    const { remessas, modoCarga } = await escolherRemessasCarga();
    ...
    if (remessas.length === 0) {
      ...
      await registrarLogEtl({
        ...
        registros: 0,
        duracaoMs: duracao,
        mensagem: "Nenhuma remessa encontrada para carga",
      });
    ...
    const descricaoModo =
      modoCarga === "inicial"
        ...
        : `incremental — lookback ${LOOKBACK_REMESSAS} remessa(s)`;
    ...
    const duracao = Date.now() - inicio;
    console.log(
      `  OK - ETL concluído em ${duracao}ms | lidos: ${totalLidos} | gravados/atualizados: ${totalGravados} | remessas: ${remessas.length}`,
    );
    ...
    await registrarLogEtl({
      ...
      duracaoMs: duracao,
      mensagem: `${descricaoModo} — ${remessas.length} remessa(s) processada(s)`,
    });
    await finalizarCargaEtl({
      idCarga,
      status: "ok",
