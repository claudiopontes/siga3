...
-- Materialized view para acelerar painéis em períodos longos.
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_mv_receita_publica_entidade_mensal_pk
...
GRANT SELECT ON public.mv_receita_publica_entidade_mensal TO anon, authenticated;

-- RPC para o painel consumir dados já filtrados no banco.
...
GRANT EXECUTE ON FUNCTION public.fn_receita_publica_entidade_mensal(INTEGER, INTEGER, BIGINT, BIGINT)
  TO anon, authenticated;
...
AS $$
BEGIN
  REFRESH MATERIALIZED VIEW public.mv_receita_publica_entidade_mensal;
END;
$$;

GRANT EXECUTE ON FUNCTION public.fn_refresh_mv_receita_publica_entidade_mensal()
  TO anon, authenticated;
